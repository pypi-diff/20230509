# Comparing `tmp/terminut-0.0.0.6.tar.gz` & `tmp/terminut-0.0.0.600.tar.gz`

## filetype from file(1)

```diff
@@ -1 +1 @@
-gzip compressed data, was "terminut-0.0.0.6.tar", last modified: Mon May  8 22:54:58 2023, max compression
+gzip compressed data, was "terminut-0.0.0.600.tar", last modified: Mon May  8 23:04:13 2023, max compression
```

## Comparing `terminut-0.0.0.6.tar` & `terminut-0.0.0.600.tar`

### file list

```diff
@@ -1,16 +1,16 @@
-drwxrwxrwx   0        0        0        0 2023-05-08 22:54:58.809600 terminut-0.0.0.6/
--rw-rw-rw-   0        0        0     1085 2023-05-08 01:47:46.000000 terminut-0.0.0.6/LICENSE
--rw-rw-rw-   0        0        0     1994 2023-05-08 22:54:58.808604 terminut-0.0.0.6/PKG-INFO
--rw-rw-rw-   0        0        0      816 2023-05-08 01:53:38.000000 terminut-0.0.0.6/README.md
--rw-rw-rw-   0        0        0       86 2023-05-08 02:01:12.000000 terminut-0.0.0.6/pyproject.toml
--rw-rw-rw-   0        0        0       42 2023-05-08 22:54:58.809600 terminut-0.0.0.6/setup.cfg
--rw-rw-rw-   0        0        0     1598 2023-05-08 22:54:47.000000 terminut-0.0.0.6/setup.py
-drwxrwxrwx   0        0        0        0 2023-05-08 22:54:58.793037 terminut-0.0.0.6/terminut/
--rw-rw-rw-   0        0        0      335 2023-05-08 02:35:20.000000 terminut-0.0.0.6/terminut/__init__.py
--rw-rw-rw-   0        0        0     2457 2023-05-08 22:54:30.000000 terminut-0.0.0.6/terminut/console.py
-drwxrwxrwx   0        0        0        0 2023-05-08 22:54:58.807606 terminut-0.0.0.6/terminut.egg-info/
--rw-rw-rw-   0        0        0     1994 2023-05-08 22:54:58.000000 terminut-0.0.0.6/terminut.egg-info/PKG-INFO
--rw-rw-rw-   0        0        0      241 2023-05-08 22:54:58.000000 terminut-0.0.0.6/terminut.egg-info/SOURCES.txt
--rw-rw-rw-   0        0        0        1 2023-05-08 22:54:58.000000 terminut-0.0.0.6/terminut.egg-info/dependency_links.txt
--rw-rw-rw-   0        0        0       33 2023-05-08 22:54:58.000000 terminut-0.0.0.6/terminut.egg-info/requires.txt
--rw-rw-rw-   0        0        0        9 2023-05-08 22:54:58.000000 terminut-0.0.0.6/terminut.egg-info/top_level.txt
+drwxrwxrwx   0        0        0        0 2023-05-08 23:04:13.046951 terminut-0.0.0.600/
+-rw-rw-rw-   0        0        0     1085 2023-05-08 01:47:46.000000 terminut-0.0.0.600/LICENSE
+-rw-rw-rw-   0        0        0     1996 2023-05-08 23:04:13.045954 terminut-0.0.0.600/PKG-INFO
+-rw-rw-rw-   0        0        0      816 2023-05-08 01:53:38.000000 terminut-0.0.0.600/README.md
+-rw-rw-rw-   0        0        0       86 2023-05-08 02:01:12.000000 terminut-0.0.0.600/pyproject.toml
+-rw-rw-rw-   0        0        0       42 2023-05-08 23:04:13.046951 terminut-0.0.0.600/setup.cfg
+-rw-rw-rw-   0        0        0     1600 2023-05-08 23:03:58.000000 terminut-0.0.0.600/setup.py
+drwxrwxrwx   0        0        0        0 2023-05-08 23:04:13.030800 terminut-0.0.0.600/terminut/
+-rw-rw-rw-   0        0        0      335 2023-05-08 02:35:20.000000 terminut-0.0.0.600/terminut/__init__.py
+-rw-rw-rw-   0        0        0     2574 2023-05-08 23:02:03.000000 terminut-0.0.0.600/terminut/console.py
+drwxrwxrwx   0        0        0        0 2023-05-08 23:04:13.045954 terminut-0.0.0.600/terminut.egg-info/
+-rw-rw-rw-   0        0        0     1996 2023-05-08 23:04:13.000000 terminut-0.0.0.600/terminut.egg-info/PKG-INFO
+-rw-rw-rw-   0        0        0      241 2023-05-08 23:04:13.000000 terminut-0.0.0.600/terminut.egg-info/SOURCES.txt
+-rw-rw-rw-   0        0        0        1 2023-05-08 23:04:13.000000 terminut-0.0.0.600/terminut.egg-info/dependency_links.txt
+-rw-rw-rw-   0        0        0       33 2023-05-08 23:04:13.000000 terminut-0.0.0.600/terminut.egg-info/requires.txt
+-rw-rw-rw-   0        0        0        9 2023-05-08 23:04:13.000000 terminut-0.0.0.600/terminut.egg-info/top_level.txt
```

### Comparing `terminut-0.0.0.6/LICENSE` & `terminut-0.0.0.600/LICENSE`

 * *Files identical despite different names*

### Comparing `terminut-0.0.0.6/PKG-INFO` & `terminut-0.0.0.600/PKG-INFO`

 * *Files 0% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: terminut
-Version: 0.0.0.6
+Version: 0.0.0.600
 Summary: Terminut // vast#1337
 Home-page: http://pypi.python.org/pypi/terminut
 Author: vast#1337
 Author-email: vastcord@proton.me
 License: MIT
 Project-URL: Homepage, https://github.com/imvast/terminut
 Project-URL: Suggestions, https://github.com/imvast/terminut/issues
```

### Comparing `terminut-0.0.0.6/README.md` & `terminut-0.0.0.600/README.md`

 * *Files identical despite different names*

### Comparing `terminut-0.0.0.6/setup.py` & `terminut-0.0.0.600/setup.py`

 * *Files 0% similar despite different names*

```diff
@@ -1,12 +1,12 @@
 #! /usr/bin/env python
 
 from setuptools import setup, find_packages
 
-vers = "0.0.0.6"
+vers = "0.0.0.600"
     
 setup(name="terminut",
       version=vers,
       description="Terminut // vast#1337",
       long_description_content_type="text/markdown",
       long_description=open("README.md", encoding="utf-8").read(),
       packages=find_packages(exclude=['tests']),
```

### Comparing `terminut-0.0.0.6/terminut/console.py` & `terminut-0.0.0.600/terminut/console.py`

 * *Files 5% similar despite different names*

```diff
@@ -1,10 +1,11 @@
 from colorama import Fore
 from datetime import datetime
 from time     import time
+from re       import sub
 
 class Settings:
     debug = True 
     timestamp = True
     c_MAIN = Fore.LIGHTBLUE_EX
     c_SECO = Fore.LIGHTBLACK_EX
 
@@ -27,30 +28,32 @@
         ("(!)" in content)
         or ("(-)" in content)
         or ("(~)" in content) 
         or ("debug" in content.lower())
         ) and (Settings.debug == False): return
     
     timestamp = f'{Settings.c_SECO}{datetime.fromtimestamp(time()).strftime("%H:%M:%S")}{Fore.RESET} ' if Settings.timestamp else ''
-    content   = timestamp + content\
+    
+    content   = sub(r'\[.*?\]', f"[{Settings.c_SECO}][{Settings.c_MAIN}]", content)
+    content   = content\
         .replace("]", f"{Settings.c_SECO}]{Fore.RESET}")\
         .replace("|", f"{Settings.c_SECO}|{Settings.c_MAIN}")\
         .replace("->", f"{Settings.c_SECO}->{Settings.c_MAIN}")\
         .replace("(+)", f"{Settings.c_SECO}({Fore.GREEN}+{Settings.c_SECO}){Settings.c_MAIN}")\
         .replace("($)", f"{Settings.c_SECO}({Fore.GREEN}${Settings.c_SECO}){Settings.c_MAIN}")\
         .replace("(-)", f"{Settings.c_SECO}({Fore.RED}-{Settings.c_SECO}){Settings.c_MAIN}")\
         .replace("(!)", f"{Settings.c_SECO}({Fore.RED}!{Settings.c_SECO}){Settings.c_MAIN}")\
         .replace("(~)", f"{Settings.c_SECO}({Fore.YELLOW}~{Settings.c_SECO}){Settings.c_MAIN}")\
         .replace("(#)", f"{Settings.c_SECO}({Fore.BLUE}#{Settings.c_SECO}){Settings.c_MAIN}")\
         .replace("(*)", f"{Settings.c_SECO}({Fore.CYAN}*{Settings.c_SECO}){Settings.c_MAIN}")
     
         # .replace("(", f"{Settings.c_SECO}({Fore.RESET}").replace(")", f"{Settings.c_SECO}){Fore.RESET}")
         # .replace("[", f"{Settings.c_SECO}[{Settings.c_MAIN}")\
         
-    return print(content, end=f"{Fore.RESET}\n")
+    return print(timestamp + content, end=f"{Fore.RESET}\n")
     
     
 @staticmethod
 def inputf(content: str):
     if "(?)" not in content: x = f"{Settings.c_SECO}({Settings.c_MAIN}?{Settings.c_SECO}){Fore.RESET} "
     else: x = ""
     content = x + content\
```

### Comparing `terminut-0.0.0.6/terminut.egg-info/PKG-INFO` & `terminut-0.0.0.600/terminut.egg-info/PKG-INFO`

 * *Files 0% similar despite different names*

```diff
@@ -1,10 +1,10 @@
 Metadata-Version: 2.1
 Name: terminut
-Version: 0.0.0.6
+Version: 0.0.0.600
 Summary: Terminut // vast#1337
 Home-page: http://pypi.python.org/pypi/terminut
 Author: vast#1337
 Author-email: vastcord@proton.me
 License: MIT
 Project-URL: Homepage, https://github.com/imvast/terminut
 Project-URL: Suggestions, https://github.com/imvast/terminut/issues
```

